...
# --- Tạo MQTT client ---
client = mqtt.Client(client_id="3276360")
...
client.will_set(
    topic="status/3276360",
    payload=json.dumps(node_info),  # gửi dưới dạng JSON string
    qos=1,
    retain=True
)
...
try:
    ...
    if connected:
        ...
        client.publish("status/3276360", msg, qos=1, retain=True)
        ...
        for i in range(1000):
            ...
            data = {
                ...
                "no2": round(random.uniform(80, 150), 2),
                "so2": round(random.uniform(1000, 2000), 2),
                "pm25": round(random.uniform(50, 120), 2),
                "pm10": round(random.uniform(200, 350), 2),
                "o3": round(random.uniform(10, 30), 2),
                "unit": "µg/m³"
            }
    ...
    node_info["status"] = "offline"
    node_info["datetimeLocal"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    client.publish("status/3276360", json.dumps(node_info), qos=1, retain=True)
